...
NSString *const kLinphoneCoreUpdate = @"UCSCoreUpdate";
...
NSString *const kLinphoneCallUpdate = @"UCSCallUpdate";
NSString *const kLinphoneRegistrationUpdate = @"UCSRegistrationUpdate";
...
- (id)init {
	if ((self = [super init])) {
		...
		NSString* factoryConfig = [LinphoneManager bundleFile:[LinphoneManager runningOnIpad]?@"bqlIPP-factory~ipad":@"bqlIPP-factory"];
		NSString *confiFileName = [LinphoneManager documentFile:@".bqlIPP"];
...
- (void)onCall:(LinphoneCall*)call StateChanged:(LinphoneCallState)state withMessage:(const char *)message {
	...
	if (!data) {
        ...
        linphone_call_set_user_data(call, data);
	}
    ...
    // 回调message参数
    NSDictionary *msgDic = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithUTF8String:message], @"message", [[BQLIPPManager InstanceIPPManager] getRemoteAddress], @"remote_address", nil];
    
    if ([BQLIPPManager InstanceIPPManager].delegate) {
        
        // 通话状态更新回调
        switch (state) {
            case BQLCallOutgoingInit:{
                [BQLIPPSDKLog saveDemoLogInfo:@"通话已呼出" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, msgDic]];
                if ([[BQLIPPManager InstanceIPPManager].delegate respondsToSelector:@selector(onOutgoingCall:withState:withMessage:)]) {
                    [[BQLIPPManager InstanceIPPManager].delegate onOutgoingCall:call withState:state withMessage:msgDic];
                }
                break;
            }
                
            case BQLCallIncomingReceived: {
                msgDic = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithUTF8String:message], @"message", [[BQLIPPManager InstanceIPPManager] getRemoteAddress], @"remote_address", [NSString stringWithUTF8String:linphone_core_get_identity([LinphoneManager getLc])], @"called_address", nil];
                [BQLIPPSDKLog saveDemoLogInfo:@"收到来电" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, msgDic]];
                if ([[BQLIPPManager InstanceIPPManager].delegate respondsToSelector:@selector(onIncomingCall:withState:withMessage:)]) {
                    [[BQLIPPManager InstanceIPPManager].delegate onIncomingCall:call withState:state withMessage:msgDic];
                }
                break;
            }
                
            case BQLCallConnected: {
                [BQLIPPSDKLog saveDemoLogInfo:@"通话已建立连接" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, msgDic]];
                if ([[BQLIPPManager InstanceIPPManager].delegate respondsToSelector:@selector(onAnswer:withState:withMessage:)]) {
                    [[BQLIPPManager InstanceIPPManager].delegate onAnswer:call withState:state withMessage:msgDic];
                }
                break;
            }
                
            case BQLCallReleased:
            case BQLCallEnd: {
                [BQLIPPSDKLog saveDemoLogInfo:@"通话结束" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, msgDic]];
                if ([[BQLIPPManager InstanceIPPManager].delegate respondsToSelector:@selector(onHangUp:withState:withMessage:)]) {
                    [[BQLIPPManager InstanceIPPManager].delegate onHangUp:call withState:state withMessage:msgDic];
                }
                break;
            }
                
            case BQLCallEarlyUpdating:
            case BQLCallEarlyUpdatedByRemote: {
                [BQLIPPSDKLog saveDemoLogInfo:@"通话未应答" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, msgDic]];
                if ([[BQLIPPManager InstanceIPPManager].delegate respondsToSelector:@selector(onHangUp:withState:withMessage:)]) {
                    [[BQLIPPManager InstanceIPPManager].delegate onHangUp:call withState:state withMessage:msgDic];
                }
                break;
            ...
            default:
                [BQLIPPSDKLog saveDemoLogInfo:@"通话状态更新" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, msgDic]];
                break;
        }
    }
}
...
- (void)onRegister:(LinphoneCore *)lc cfg:(LinphoneProxyConfig*) cfg state:(LinphoneRegistrationState) state message:(const char*) message {
    ...
    [BQLIPPSDKLog saveDemoLogInfo:@"登陆状态更新" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, [NSString stringWithUTF8String:message]]];
    
    // 处理回调
    if ([BQLIPPManager InstanceIPPManager].delegate  && [[BQLIPPManager InstanceIPPManager].delegate respondsToSelector:@selector(onRegisterStateChange:message:)]) {
        return [[BQLIPPManager InstanceIPPManager].delegate onRegisterStateChange:state message:message];
...
- (void)cancelLocalNotifTimerForCallId:(NSString*)callid {
    ...
    bctbx_list_t *calls = (bctbx_list_t *)linphone_core_get_calls(theLinphoneCore);
    bctbx_list_t *call = bctbx_list_find_custom(calls, (MSCompareFunc)comp_call_id, [callid UTF8String]);
    if (call != NULL) {
...
//    };
    bctbx_list_t *calls = (bctbx_list_t *)linphone_core_get_calls(theLinphoneCore);
    if(bctbx_list_find_custom(calls, (MSCompareFunc)comp_call_id, [callid UTF8String])) {
        Linphone_warn(@"Call id [%@] already handled",callid);
        return;
    }
...
//	}
    const bctbx_list_t *callList = (bctbx_list_t *)linphone_core_get_calls(theLinphoneCore);
    if(!currentCall //no active call
       && callList // at least one call in a non active state
       && bctbx_list_find_custom(callList, (MSCompareFunc) comp_call_state_paused, NULL)) {
        [self startCallPausedLongRunningTask];
    }
...
- (void)copyDefaultSettings {
    ...
    // linphonerc
	NSString *src = [LinphoneManager bundleFile:[LinphoneManager runningOnIpad]?@"bqlIPP~ipad":@"bqlIPP"];
	NSString *dst = [LinphoneManager documentFile:@".bqlIPP"];
...
+ (int)unreadMessageCount {
    int count = 0;
    MSList* rooms = (MSList *)linphone_core_get_chat_rooms([LinphoneManager getLc]);
    MSList* item = rooms;
    while (item) {
